    ...
    describe('Basic functions', () => {
        describe('Constants', () => {
            ...
            it('Returns minimum amount to stake for a Validator', async () => {
                expect((await this.sfc.minSelfStake()).toString()).to.equals('200000000000000000000000');
            });
            ...
            it('Returns the period of time that stake is locked', async () => {
                expect((await this.sfc.withdrawalPeriodTime()).toString()).to.equals('86400');
            });

            it('Returns the number of epochs that stake is locked', async () => {
                expect((await this.sfc.withdrawalPeriodEpochs()).toString()).to.equals('6');
            });
            ...
            it('Should reject if amount is insufficient for self-stake', async () => {
                ...
                await expect(this.sfc.createValidator(pubkey, {
                    from: secondValidator,
                    value: amount18('0.3'),
                })).to.be.rejectedWith('VM Exception while processing transaction: revert insufficient self-stake');
            });
        ...
        describe('Events emitters', () => {
            ...
            it('Should call updateOfflinePenaltyThreshold', async () => {
                await this.sfc.updateOfflinePenaltyThreshold(20, 1200);
            });
        });
    });
});
    ...
    describe('Create validators', () => {
        ...
        it('Should not be able to stake if Validator not created yet', async () => {
            await expect(this.sfc.delegate(1, {
                from: firstDelegator,
                value: amount18('200000'),
            })).to.be.rejectedWith('VM Exception while processing transaction: revert validator doesn\'t exist');
            await expect(this.sfc.createValidator(pubkey, {
                from: firstValidator,
                value: amount18('200000'),
            })).to.be.fulfilled;
        ...
        it('Should stake with different delegators', async () => {
            ...
            expect(stakes[0].amount).eq(amount18('200000').toString())
            ...
            expect(stakes[2].amount).eq(amount18('205000').toString())
            ...
            expect(stakes[4].amount).eq(amount18('210000').toString())
        ...
        it('Should return the total of received Stake', async () => {
            ...
            expect(validator.receivedStake.toString()).to.equals('200027000000000000000000');
        });
    });
});
...
contract('SFC', async ([firstValidator, secondValidator, thirdValidator, firstDelegator, secondDelegator, thirdDelegator]) => {
    describe('Returns Validator', () => {
        ...
        beforeEach(async () => {
            ...
            await this.sfc.enableNonNodeCalls();
            this.node = new BlockchainNode(this.sfc, firstValidator);
            await expect(this.sfc.createValidator(pubkey, { from: firstValidator, value: amount18('200000') })).to.be.fulfilled;
            await this.sfc.delegate(1, { from: firstDelegator, value: amount18('11000') });
            await this.sfc.delegate(1, { from: secondDelegator, value: amount18('8000') });
            await this.sfc.delegate(1, { from: thirdDelegator, value: amount18('8000') });
        ...
        it('Should returns Validator\'s Received Stake', async () => {
            expect(validator.receivedStake.toString()).to.equals('227000000000000000000000');
        });
    ...
    describe('EpochSnapshot', () => {
        ...
        beforeEach(async () => {
            ...
            this.node = new BlockchainNode(this.sfc, firstValidator);
            await expect(this.sfc.createValidator(pubkey, { from: firstValidator, value: amount18('200000') })).to.be.fulfilled;
            await this.sfc.delegate(1, { from: firstDelegator, value: amount18('1100') });
            await this.sfc.delegate(1, { from: secondDelegator, value: amount18('8000') });
            await this.sfc.delegate(1, { from: thirdDelegator, value: amount18('8000') });
            validator = await this.sfc.getValidator(1);
        });
    ...
    describe('Methods tests', async () => {
        ...
        it('checking createValidator function', async () => {
            ...
            expect(await this.sfc.totalStake.call()).to.be.bignumber.equal(amount18('405000'));
            ...
            expect(firstValidatorObj.receivedStake).to.be.bignumber.equal(amount18('200000'));
            ...
            expect(secondValidatorObj.receivedStake).to.be.bignumber.equal(amount18('205000'));
            ...
            expect(await this.sfc.getStake.call(firstValidator, firstValidatorID)).to.be.bignumber.equal(amount18('200000'));
            expect(await this.sfc.getStake.call(secondValidator, secondValidatorID)).to.be.bignumber.equal(amount18('205000'));

            // check fired node-related logs
            expect(Object.keys(this.node.nextValidators).length).to.equal(2);
            expect(this.node.nextValidators[firstValidatorID.toString()]).to.be.bignumber.equal(amount18('200000'));
            expect(this.node.nextValidators[secondValidatorID.toString()]).to.be.bignumber.equal(amount18('205000'));
        });
        ...
        it('checking sealing epoch', async () => {
            ...
            expect(this.node.validators[firstValidatorID.toString()]).to.be.bignumber.equal(amount18('200000'));
            expect(this.node.validators[secondValidatorID.toString()]).to.be.bignumber.equal(amount18('205000'));
            expect(Object.keys(this.node.nextValidators).length).to.equal(3);
            expect(this.node.nextValidators[firstValidatorID.toString()]).to.be.bignumber.equal(amount18('200001'));
            expect(this.node.nextValidators[secondValidatorID.toString()]).to.be.bignumber.equal(amount18('205000'));
            expect(this.node.nextValidators[thirdValidatorID.toString()]).to.be.bignumber.equal(amount18('204000'));
        });
    });
});
...
contract('SFC', async ([firstValidator, secondValidator, thirdValidator, testValidator, firstDelegator, secondDelegator, account1, account2, account3, account4]) => {
    ...
    describe('Staking / Sealed Epoch functions', () => {
        it('Should return claimed Rewards until Epoch', async () => {
            ...
            const comission = (await this.sfc.validatorCommission()).toString()
            if (comission != "1000000000000000000") {
                ...
                await expectRevert(this.sfc.claimRewards(1, { from: firstDelegator }), 'zero rewards');
            }
        });
        ...
        it('Check pending Rewards of delegators', async () => {
            ...
            const expectedFullRewardFirstValidator = 9720
            const validatorCommission = await this.sfc.validatorCommission()

            const firstValidatorReward = validatorCommission.mul(new BN(expectedFullRewardFirstValidator)).div(new BN(amount18('1')))
            const firstDelegatorReward = (new BN(amount18('1')).sub(validatorCommission)).mul(new BN(expectedFullRewardFirstValidator)).div(new BN(amount18('1')))

            expect((await this.sfc.pendingRewards(firstValidator, firstValidatorID)).toString()).to.equals(firstValidatorReward.toString());
            expect((await this.sfc.pendingRewards(firstDelegator, firstValidatorID)).toString()).to.equals(firstDelegatorReward.toString());
        });

        it('Check if pending Rewards have been increased after sealing Epoch', async () => {
            await this.sfc.updateBaseRewardPerSecond(new BN('1'));

            const firstValidatorRewardPerEpoch = 9720
            const validatorCommission = await this.sfc.validatorCommission()
            const firstValidatorReward = validatorCommission.mul(new BN(firstValidatorRewardPerEpoch)).div(new BN(amount18('1')))
            const firstDelegatorReward = (new BN(amount18('1')).sub(validatorCommission)).mul(new BN(firstValidatorRewardPerEpoch)).div(new BN(amount18('1')))
            ...
            await sealEpoch(this.sfc, (new BN(60 * 60 * 24)).toString());
            expect((await this.sfc.pendingRewards(firstValidator, firstValidatorID)).toString()).to.equals(firstValidatorReward.mul(new BN(2)).toString());
            expect((await this.sfc.pendingRewards(firstDelegator, firstValidatorID)).toString()).to.equals(firstDelegatorReward.mul(new BN(2)).toString());
        });
        ...
        it('Should increase balances after claiming Rewards', async () => {
            ...
            expect(firstDelegatorPendingRewards.toString()).to.equals(firstDelegatorReward.toString());
            const firstDelegatorBalance = new BN(await web3.eth.getBalance(firstDelegator));

            if (validatorCommission != "1000000000000000000") {
                await this.sfc.claimRewards(1, { from: firstDelegator });
        ...
        it('Should increase stake after restaking Rewards', async () => {
            ...
            if (validatorCommission != "1000000000000000000") {
                ...
                await expectRevert(this.sfc.restakeRewards(1, { from: firstDelegator }), 'zero rewards');
            }
        });
        ...
        it('Should increase locked stake after restaking Rewards', async () => {
            ...
            const firstValidatorRewardPerEpoch = 9720
            const validatorCommission = await this.sfc.validatorCommission()
            const firstValidatorReward = validatorCommission.mul(new BN(firstValidatorRewardPerEpoch)).div(new BN(amount18('1'))).mul(new BN(205)).div(new BN(100))
            const firstDelegatorReward = (new BN(amount18('1')).sub(validatorCommission)).mul(new BN(firstValidatorRewardPerEpoch)).div(new BN(amount18('1'))).mul(new BN(11166)).div(new BN(10000))
            ...
            const firstDelegatorPendingRewards = await this.sfc.pendingRewards(firstDelegator, firstValidatorID);
            expect(firstDelegatorPendingRewards).to.be.bignumber.equal(firstDelegatorReward);

            const firstDelegatorPendingLockupRewards = firstDelegatorReward
            const firstDelegatorStake = await this.sfc.getStake(firstDelegator, firstValidatorID);
            const firstDelegatorLockupInfo = await this.sfc.getLockupInfo(firstDelegator, firstValidatorID);
        ...
        it('Should return stashed Rewards', async () => {
            ...
            if (validatorCommission != "1000000000000000000") {
                ...
                await expectRevert(this.sfc.stashRewards(firstDelegator, 1), 'nothing to stash');
            }
            expect((await this.sfc.rewardsStash(firstDelegator, 1)).toString()).to.equals(firstDelegatorReward.toString());
        });

        it('Should update the validator on node', async () => {
            await expectRevert(this.sfc.updateOfflinePenaltyThreshold(19, 1200), "too low penalty blocks num")
            await expectRevert(this.sfc.updateOfflinePenaltyThreshold(20, 1199), "too low penalty time")
            await this.sfc.updateOfflinePenaltyThreshold(500, 10000);

            expect((await this.sfc.offlinePenaltyThreshold()).blocksNum).to.bignumber.equals(new BN(500));
            expect((await this.sfc.offlinePenaltyThreshold()).time).to.bignumber.equals(new BN(10000));
        });
    ...
    describe('Stake lockup', () => {
        beforeEach('lock stakes', async () => {
            // Lock 75% of stake for 60% of a maximum lockup period
            // Should receive (0.3 * 0.25 + (0.3 + 0.7 * 0.6) * 0.75) / 0.3 = 2.05 times more rewards
            await this.sfc.lockStake(firstValidatorID, new BN(86400 * 219), amount18('600000'), {
                from: firstValidator,
            });
            // Lock 25% of stake for 20% of a maximum lockup period
            // Should receive (0.3 * 0.75 + (0.3 + 0.7 * 0.2) * 0.25) / 0.3 = 1.1166 times more rewards
            await this.sfc.lockStake(firstValidatorID, new BN(86400 * 73), amount18('100000'), {
                from: firstDelegator,
            });
        });
        ...
        it('Should return pending rewards after unlocking and re-locking', async () => {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                const stepValidatorRewardNoLock = validatorCommission.mul(new BN(firstValidatorRewardPerEpoch)).div(new BN(amount18('1')))
                const stepDelegatorRewardNoLock = (new BN(amount18('1')).sub(validatorCommission)).mul(new BN(firstValidatorRewardPerEpoch)).div(new BN(amount18('1')))

                const stepValidatorReward = stepValidatorRewardNoLock.mul(new BN(205)).div(new BN(100))
                const stepDelegatorReward = stepDelegatorRewardNoLock.mul(new BN(11166)).div(new BN(10000))

                const firstValidatorReward = stepValidatorReward.mul(new BN(73))
                const firstDelegatorReward = stepDelegatorReward.mul(new BN(73))

                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(firstDelegatorReward);
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(firstValidatorReward);
                ...
                const firstValidatorReward2 = firstValidatorReward.add(stepValidatorReward)
                const firstDelegatorReward2 = firstDelegatorReward.add(stepDelegatorReward)

                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(firstDelegatorReward2);
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(firstValidatorReward2);
                expect(await this.sfc.highestLockupEpoch(firstDelegator, 1)).to.be.bignumber.equal(epoch.add(new BN(1)));
                ...
                const firstValidatorReward3 = firstValidatorReward2.add(stepValidatorReward.mul(new BN(145)))
                const firstDelegatorReward3 = firstDelegatorReward2.add(stepDelegatorReward.mul(new BN(145)))

                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(firstDelegatorReward3);
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(firstValidatorReward3);
                expect(await this.sfc.highestLockupEpoch(firstDelegator, 1)).to.be.bignumber.equal(epoch.add(new BN(1)));
                ...
                const firstValidatorReward4 = firstValidatorReward3.add(stepValidatorRewardNoLock)
                const firstDelegatorReward4 = firstDelegatorReward3.add(stepDelegatorRewardNoLock)

                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(new BN(firstDelegatorReward4));
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(new BN(firstValidatorReward4));
                expect(await this.sfc.highestLockupEpoch(firstDelegator, 1)).to.be.bignumber.equal(epoch.add(new BN(1)));
                expect(await this.sfc.highestLockupEpoch(firstValidator, 1)).to.be.bignumber.equal(epoch.add(new BN(3)));
                ...
                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(firstDelegatorReward4);
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(firstValidatorReward4);
                ...
                if (firstDelegatorReward4.toString() != '0') {
                    await this.sfc.claimRewards(1, { from: firstDelegator });
                } else {
                    await expectRevert(this.sfc.claimRewards(1, { from: firstDelegator }), 'zero rewards');
                }

                if (firstValidatorReward4.toString() != '0') {
                    await this.sfc.claimRewards(1, { from: firstValidator });
                } else {
                    await expectRevert(this.sfc.claimRewards(1, { from: firstValidator }), 'zero rewards');
                }
            }
        });
    });
...
contract('SFC', async ([firstValidator, testValidator, firstDelegator, secondDelegator, thirdDelegator, account1, account2, account3]) => {
    ...
    beforeEach(async () => {
        ...
        await this.sfc.createValidator(pubkey, {
            from: account1,
            value: amount18('10000000'),
        });

        await this.sfc.createValidator(pubkey, {
            from: account2,
            value: amount18('5000000'),
        });

        await this.sfc.createValidator(pubkey, {
            from: account3,
            value: amount18('1000000'),
        });
        ...
        await this.sfc.lockStake(testValidator3ID, (60 * 60 * 24 * 364), amount18('1000000'),
            { from: account3 });
    ...
    describe('Test Rewards Calculation', () => {
        ...
        it('Get stakes/get wr requests should correctly work after undelegate', async () => {
            ...
            expect(stakes[3].amount).eq(amount18('500000').toString())
            ...
            await this.sfc.delegate(testValidator3ID, {
                from: secondDelegator,
                value: amount18('110000'),
            });

            stakes = (await this.sfc.getStakes(0, 10)).slice(1)
            expect(stakes[3].delegator).eq(thirdDelegator)
            expect(stakes[3].validatorId).eq(testValidator3ID.toString())
            expect(stakes[3].amount).eq(amount18('1000000').toString())

            await this.sfc.undelegate(testValidator3ID, amount18('700000'), { from: thirdDelegator })

            let wrRequests = await this.sfc.getWrRequests(thirdDelegator, testValidator3ID, 0, 5)
            expect(wrRequests[0].amount).eq(amount18('700000').toString())

            stakes = (await this.sfc.getStakes(0, 10)).slice(1)
            expect(stakes[3].delegator).eq(thirdDelegator)
            expect(stakes[3].validatorId).eq(testValidator3ID.toString())
            expect(stakes[3].amount).eq(amount18('300000').toString())

            expect(stakes[4].delegator).eq(secondDelegator)
            expect(stakes[4].validatorId).eq(testValidator3ID.toString())
            expect(stakes[4].amount).eq(amount18('110000').toString())

            await this.sfc.undelegate(testValidator3ID, amount18('300000'), { from: thirdDelegator })

            wrRequests = await this.sfc.getWrRequests(thirdDelegator, testValidator3ID, 0, 5)
            expect(wrRequests[0].amount).eq(amount18('700000').toString())
            expect(wrRequests[1].amount).eq(amount18('300000').toString())

            stakes = (await this.sfc.getStakes(0, 10)).slice(1)
            expect(stakes[3].delegator).eq(secondDelegator)
            expect(stakes[3].validatorId).eq(testValidator3ID.toString())
            expect(stakes[3].amount).eq(amount18('110000').toString())
            ...
            expect(stakes[3].delegator).eq(secondDelegator)
            expect(stakes[3].validatorId).eq(testValidator3ID.toString())
            expect(stakes[3].amount).eq(amount18('220000').toString())

            await this.sfc.undelegate(testValidator3ID, amount18('220000'), { from: secondDelegator })
            stakes = (await this.sfc.getStakes(0, 10)).slice(1)
            expect(stakes[3].delegator).eq(zeroAddr)
            expect(stakes[3].validatorId).eq('0')
            expect(stakes[3].amount).eq('0')

            expect(stakes[2].delegator).eq(account3)
            expect(stakes[2].validatorId).eq(testValidator3ID.toString())
            expect(stakes[2].amount).eq(amount18('1000000').toString())
        });
...
contract('SFC', async ([firstValidator, testValidator, firstDelegator, secondDelegator, thirdDelegator, account1, account2, account3]) => {
    ...
    describe('Test Calculation Rewards with Lockup', () => {
        ...
        it('Should be able to lock for 1 month', async () => {
            ...
            await this.sfc.lockStake(testValidator3ID, (60 * 60 * 24 * 14), amount18('1000000'),
                { from: thirdDelegator });

            await sealEpoch(this.sfc, (new BN(60 * 60 * 24 * 14)).toString());
        });

        it('Should not unlock if not locked up VC', async () => {
            await sealEpoch(this.sfc, (new BN(1000)).toString());
        ...
        it('Should not be able to unlock more than locked stake', async () => {
            ...
            await expectRevert(this.sfc.unlockStake(testValidator3ID, amount18('10000000'), { from: thirdDelegator }), 'not enough locked stake');
        });

        it('Unlocking penalty should be 0 for delegator', async () => {
                ...
                expect(await this.sfc.unlockStake.call(testValidator3ID, amount18('1000000'), { from: thirdDelegator })).to.be.bignumber.equal(amount18('0'));
                expect(await this.sfc.unlockStake.call(testValidator3ID, amount18('500000'), { from: thirdDelegator })).to.be.bignumber.equal(amount18('0'));
                expect(await this.sfc.unlockStake.call(testValidator3ID, amount18('10000'), { from: thirdDelegator })).to.be.bignumber.equal(amount18('0'));
                await this.sfc.unlockStake(testValidator3ID, amount18('500000'), { from: thirdDelegator });
                await expectRevert(this.sfc.unlockStake(testValidator3ID, amount18('510000'), { from: thirdDelegator }), 'not enough locked stake');
                expect(await this.sfc.unlockStake.call(testValidator3ID, amount18('500000'), { from: thirdDelegator })).to.be.bignumber.equal(amount18('0'));
                expect(await this.sfc.unlockStake.call(testValidator3ID, amount18('10000'), { from: thirdDelegator })).to.be.bignumber.equal(amount18('0'));
            
        });

        it('Should unlock after period ended and stash rewards will be reverted', async () => {
                await sealEpoch(this.sfc, (new BN(1000)).toString());
                ...
                expect(unlockedStake.toString()).to.equal('10000000000000000000000000');
                ...
                expect(unlockedStake.toString()).to.equal('9000000000000000000000000');
                expect(web3.utils.fromWei(pendingRewards.toString(), 'ether')).to.equal('0');
                await sealEpoch(this.sfc, (new BN(60 * 60 * 24 * 14)).toString());
